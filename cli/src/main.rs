...
async fn main() -> anyhow::Result<()> {
    ...
    if args.len() > 1 {
        match args[1].as_str() {
            ...
            "--language" => { if let Some(v) = args.get(2) { cfg.set_lang(v); cfg.save(); println!("Language: {}", v); } }
            "--ui-language" => { if let Some(v) = args.get(2) { cfg.set_ui_lang(v); cfg.save(); println!("UI: {}", v); } }
    ...
    let history_file = dirs::data_dir().unwrap_or_else(|| std::path::PathBuf::from(".")).join("g0d").join("history.txt");
    ...
    let mut editor = Reedline::create()
        ...
        .with_history(Box::new(
            reedline::FileBackedHistory::with_file(1000, history_file)
                .unwrap_or_else(|_| reedline::FileBackedHistory::default())
        ))
    ...
    loop {
        let prompt_text = match mode.as_str() {
            "godmode" => term.y("godmode › "),
            "parseltongue" => term.g("snake › "),
            "ultra" => term.c("ultra › "),
            _ => term.d(&format!("{} › ", current_dir_name())),
        };
        ...
        match sig {
            Ok(Signal::Success(line)) => {
                ...
                if input.starts_with('/') {
                    ...
                    match parts[0] {
                        "/exit" | "/quit" => { println!("{}", term.m("bye")); break; }
                        "/help" => print_slash_help(&term),
                        "/key" => cmd_key(&mut cfg, parts.get(1).copied(), &term),
                        "/provider" => cmd_provider(&mut cfg, parts.get(1).copied(), parts.get(2).copied(), parts.get(3).copied(), &term),
                        "/model" => cmd_model(&mut cfg, parts.get(1).copied(), &term),
                        "/providers" => cmd_providers_list(&cfg, &term),
                        "/status" => cmd_status(&cfg, &mode, &term),
                        "/chat" => { mode = "chat".into(); println!("{}", term.g("Chat mode")); }
                        "/godmode" => { mode = "godmode".into(); println!("{}", term.y("GODMODE — 5 models")); }
                        "/snake" => { mode = "parseltongue".into(); println!("{}", term.g("Parseltongue")); }
                        "/ultra" => { mode = "ultra".into(); println!("{}", term.c("ULTRAPLINIAN")); }
                        "/language" => cmd_language(&mut cfg, parts.get(1).copied(), &term),
                        _ => {
                            let suggestions = commands::registry().iter()
                                .filter(|c| fuzzy_match(c.name, &input))
                                .map(|c| c.name).take(3).collect::<Vec<_>>();
                            if suggestions.is_empty() {
                                println!("{}", term.d(&format!("Unknown. Type /help")));
                            } else {
                ...
                let key = match cfg.get_api_key() {
                    Ok(k) => k, Err(e) => { println!("{}", term.y(&e.to_string())); println!("{}", term.d(&format!("Get key at https://openrouter.ai/keys"))); println!("{}", term.d("/key sk-or-v1-...")); continue; }
                };
                let endpoint = cfg.get_endpoint();

                println!("{}", term.dim_line());
                match mode.as_str() {
            ...
            Ok(Signal::CtrlD) | Ok(Signal::CtrlC) => { println!("{}", term.m("bye")); break; }
...
fn banner(cfg: &config::Config, term: &terminal::TerminalState) {
    ...
    println!("{}  {}  |  {}", term.d(""), ctx.cwd, ctx.project_type);
    if let Some(ref branch) = ctx.git_branch {
        let status = ctx.git_status.as_deref().unwrap_or("");
        print!("{}  {}", term.d(""), branch);
        if !status.is_empty() { print!("{}  [{}]", term.d(""), status); }
        println!();
    }
    let has_key = cfg.get_api_key().is_ok();
    if !has_key {
        println!("\n{}", term.y("No API key. Setup:"));
        println!("{}  1. Get key at https://openrouter.ai/keys", term.d(""));
        println!("{}  2. /key sk-or-v1-...", term.d(""));
        println!();
    }
    // Show hint bar
    if has_key {
        println!("{}", term.d("Type /help for commands, ask anything to begin"));
    }
    println!();
}

fn print_help(term: &terminal::TerminalState) {
    println!("g0d — AI coding agent\n");
...
fn cmd_key(cfg: &mut config::Config, arg: Option<&str>, term: &terminal::TerminalState) {
    if let Some(key) = arg {
        ...
        println!("{}", term.d("Paste your key: /key sk-or-v1-..."));
    }
}
...
fn cmd_provider(cfg: &mut config::Config, sub: Option<&str>, arg1: Option<&str>, arg2: Option<&str>, term: &terminal::TerminalState) {
    match sub {
        ...
        Some("default") => if let Some(id) = arg1 { cfg.set_default_provider(id); cfg.save(); println!("{}", term.g(&format!("Now using {id}"))); }
...
fn cmd_providers_mini(cfg: &config::Config, term: &terminal::TerminalState) {
    for p in &cfg.providers {
        ...
        println!("{}  {dot} {star} {}", term.d(""), p.id);
    }
    println!("{}  * = current, ● = key set", term.d(""));
}
...
fn cmd_providers_list(cfg: &config::Config, term: &terminal::TerminalState) {
    for p in &cfg.providers {
        ...
        println!("{}  {star} {:<14} {}  {}", term.d(""), p.id, key_status, term.d(&p.endpoint));
    }
}
...
fn cmd_status(cfg: &config::Config, mode: &str, term: &terminal::TerminalState) {
    ...
    println!("{}  Mode: {}  |  Provider: {}  |  Model: {}", term.d(""), mode, p.id, cfg.default_model());
    println!("{}  Key: {}  |  Language: {}  |  Config: {}", term.d(""), if has_key { "set" } else { "not set" }, lang, config::config_path().display());
}
...
fn cmd_language(cfg: &mut config::Config, arg: Option<&str>, term: &terminal::TerminalState) {
    match arg {
        ...
        _ => { println!("{}  /language auto | vi | en", term.d("")); }
    }
}

// ── API call functions ─────────────────────────────────────────

async fn cmd_chat(cfg: &config::Config, query: &str, term: &terminal::TerminalState) -> anyhow::Result<()> {
    let key = cfg.get_api_key()?; let endpoint = cfg.get_endpoint();
    run_chat(&key, &endpoint, query, &cfg.default_model(), term).await
}

async fn cmd_godmode(cfg: &config::Config, query: &str, term: &terminal::TerminalState) -> anyhow::Result<()> {
    let key = cfg.get_api_key()?; let endpoint = cfg.get_endpoint();
    run_godmode(&key, &endpoint, query, term).await
}

async fn cmd_parseltongue(cfg: &config::Config, query: &str, term: &terminal::TerminalState) -> anyhow::Result<()> {
    let key = cfg.get_api_key()?; let endpoint = cfg.get_endpoint();
    let _ = run_parseltongue_repl(&key, &endpoint, query, term).await;
    Ok(())
}

async fn cmd_ultra(cfg: &config::Config, query: &str, tier: &str, term: &terminal::TerminalState) -> anyhow::Result<()> {
    let key = cfg.get_api_key()?; let endpoint = cfg.get_endpoint();
    run_ultra(&key, &endpoint, query, tier, term).await
}
...
async fn run_chat(key: &str, endpoint: &str, query: &str, model: &str, term: &terminal::TerminalState) -> anyhow::Result<()> {
    ...
    let ctx = context::read_context();
    let system = format!("You are g0d, a coding assistant.\n\n{}\nBe concise. Use file paths when relevant.", context::context_summary(&ctx));
        ...
        for line in String::from_utf8_lossy(&chunk).lines() {
            ...
            if let Ok(p) = serde_json::from_str::<serde_json::Value>(&line[6..]) {
                if let Some(c) = p["choices"][0]["delta"]["content"].as_str() {
                    if first { print!("{}", term.d("")); first = false; }
                    print!("{}", term.render(c));
                    std::io::stdout().flush()?;
                }
            }
...
async fn run_godmode(key: &str, endpoint: &str, query: &str, term: &terminal::TerminalState) -> anyhow::Result<()> {
    ...
    for r in &results {
        if let Ok(Ok((p, content))) = r {
            ...
            println!("{} {} ({})", term.c(&p.persona.name), p.model, term.render(&t.replace('\n', "\n    ")));
        }
    }
    Ok(())
}
...
async fn run_ultra(key: &str, endpoint: &str, query: &str, tier_name: &str, term: &terminal::TerminalState) -> anyhow::Result<()> {
    ...
    for r in futures::future::join_all(tasks).await { if let Ok(Ok((m, content))) = r { let t: String = content.chars().take(200).collect(); println!("{} {}", term.c(&m), term.render(&t.replace('\n', "\n    "))); } }
    Ok(())
}
