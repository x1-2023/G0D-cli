...
fn detect_project_type(cwd: &str) -> String {
    ...
    let checks: Vec<(&str, &str)> = vec![
        ("Cargo.toml", "Rust"), ("package.json", "Node.js"), ("tsconfig.json", "TypeScript"),
        ("go.mod", "Go"), ("requirements.txt", "Python"), ("pyproject.toml", "Python"),
        ("CMakeLists.txt", "C/C++"), ("Makefile", "C/Make"), ("pom.xml", "Java Maven"),
        ("build.gradle", "Java Gradle"), (".csproj", "C#/.NET"), (".sln", "C# Solution"),
        ("Gemfile", "Ruby"), ("mix.exs", "Elixir"), ("Dockerfile", "Docker"), (".git", "Git"),
    ];
...
fn git_status_summary() -> Option<String> {
    ...
    let parts: Vec<String> = [
        (m>0).then(|| format!("{}M", m)), (a>0).then(|| format!("{}A", a)),
        (d>0).then(|| format!("{}D", d)), (u>0).then(|| format!("{}?", u)),
...
fn directory_tree_summary(cwd: &str) -> String {
    let path = Path::new(cwd);
    let Ok(entries) = std::fs::read_dir(path) else { return "  (cannot read)\n".into(); };
